...
\noindent\rule[-0.2cm]{0.45cm}{1.2cm}\hspace{0.15cm}%
\raisebox{0.85cm}{\rule{0.85\textwidth}{0.5pt}}\\[0.3cm]
\noindent\hspace*{0.6cm}{\Huge\textbf{DEDICACE}}\\[0.8cm]
...
{\Huge\textbf{À}} mes frères et sœurs, mes complices et mes premiers amis, dont la présence chaleureuse et le soutien constant m'ont accompagnée dans les moments de doute comme dans les moments de joie.
\end{center}

\begin{center}
\textit{\textbf{À mes chers enseignants}}

{\Huge\textbf{À}} mes chers enseignants, qui m'ont transmis le savoir avec patience et passion tout au long de mon parcours académique. Leur dévouement et leurs précieux conseils ont façonné la personne que je suis devenue aujourd'hui.
\end{center}
...
% --- Décommente quand tu uploades logo_elite.png dans images/ ---
% \begin{figure}[H]
% \centering
% \includegraphics[width=0.4\textwidth]{images/logo_elite.png}
...
% --- Décommente quand tu uploades simbox.png dans images/ ---
% \begin{figure}[H]
% \centering
% \includegraphics[width=0.5\textwidth]{images/simbox.png}
...
\item \textbf{Random Forest} : utilise plusieurs arbres de décision qui votent ensemble pour donner une prédiction fiable.
 ...
 \colorbox{yellow!30}{\textbf{Exemple dans notre projet :}} on entraîne le Random Forest sur les CDR étiquetés. Chaque arbre analyse un sous-ensemble de features (volume d'appels, ratio MO/MT, durée moyenne, nombre d'IMEI). Pour un nouveau MSISDN, si 70 arbres sur 100 votent « fraude », le numéro est classé comme SIM Box.
\vspace{0.5cm}
 
\item \textbf{SVM (Support Vector Machine)} : sépare les données en deux catégories grâce à une frontière mathématique optimale.

 \colorbox{yellow!30}{\textbf{Exemple dans notre projet :}} le SVM trace une frontière dans l'espace des features. D'un côté, les utilisateurs normaux (peu d'appels, durée longue, mobilité) ; de l'autre, les SIM Box (beaucoup d'appels courts, position fixe). Tout nouveau MSISDN est placé d'un côté ou de l'autre selon ses caractéristiques.

 \vspace{0.5cm}
\item \textbf{XGBoost} : combine plusieurs modèles faibles pour créer un modèle puissant et précis.

\colorbox{yellow!30}{\textbf{Exemple dans notre projet :}} XGBoost construit ses arbres les uns après les autres. Le premier arbre détecte les fraudes évidentes (ex : >500 appels/jour). Les arbres suivants se concentrent sur les erreurs du précédent (ex : SIM Box plus discrètes avec ~80 appels/jour). Résultat : meilleure détection des fraudes subtiles dans nos données CDR.
 \vspace{0.5cm}
...
\colorbox{yellow!30}{\textbf{Exemple dans notre projet :}} on applique Isolation Forest sur l'ensemble des MSISDN actifs. Les numéros qui s'isolent rapidement (en peu de coupures dans l'arbre) sont marqués comme suspects. Par exemple, un MSISDN avec 300 appels sortants en 1 heure et 0 entrant sera isolé en 2-3 étapes seulement, alors qu'un utilisateur normal demandera 10 à 15 étapes.
...
\begin{figure}[h!]
\centering
\fbox{\parbox{0.7\textwidth}{\centering\vspace{2cm}[Architecture technique]\vspace{2cm}}}
...
\hline
Product Owner & M. Ithar Lounissi (encadrant entreprise) \\
\hline
Scrum Master & M. Ridha Ejbali (encadrant académique) \\
\hline
Scrum Team & Samar Guizani (étudiante) \\
\hline
\end{tabular}
\end{table}
...
L'interface utilisateur a été conçue selon une approche centrée sur l'expérience de l'analyste, en privilégiant la lisibilité, l'efficacité opérationnelle et l'esthétique professionnelle. L'architecture du frontend repose sur le moteur de templates \textbf{Jinja2} couplé à \textbf{Flask}, permettant un rendu côté serveur rapide et une organisation modulaire des vues à travers un système d'héritage de templates.

La structure visuelle s'articule autour d'une \textbf{barre latérale de navigation} fixe, regroupant les fonctionnalités par sections thématiques (\textit{Main}, \textit{Detection}, \textit{Analyse}, \textit{Administration}), ce qui facilite l'orientation de l'utilisateur et accélère l'accès aux modules critiques. Cette barre intègre également un identifiant visuel personnalisé représentant la fraude SIM Box, ainsi qu'un badge contextuel affichant le rôle de l'utilisateur connecté (\textit{admin} ou \textit{analyste}).

Le design adopte une \textbf{palette chromatique professionnelle} déclinée autour de tonalités bleu marine et émeraude, inspirée des standards modernes des plateformes de cybersécurité et de \textit{business intelligence}. Cette palette a été choisie pour véhiculer à la fois la rigueur technique et la sérénité opérationnelle nécessaires à un outil de détection de fraude. Les composants graphiques (cartes statistiques, graphiques, tableaux) suivent un langage visuel cohérent : coins légèrement arrondis, ombres subtiles, dégradés discrets et iconographie expressive afin de hiérarchiser l'information sans surcharger l'écran.

L'interface est entièrement \textbf{responsive} et compatible avec les principaux navigateurs modernes. Les graphiques dynamiques sont générés via la bibliothèque \textbf{Chart.js}, tandis que les interactions asynchrones (chargement des suspects, vérification manuelle, prédiction en temps réel) reposent sur des appels \textbf{AJAX} permettant de fluidifier l'expérience sans rechargement de page.
...
\subsection{Page d'accueil du tableau de bord}
La page d'accueil, illustrée par la figure~\ref{fig:dashboard}, constitue le point d'entrée principal de la plateforme à l'ouverture du site. Elle a été conçue pour offrir à l'analyste une \textbf{vue d'ensemble synthétique et instantanée} de l'état du système de détection, conformément aux principes ergonomiques des tableaux de bord décisionnels.

Sa partie supérieure présente une série de \textbf{cartes statistiques (KPI cards)} mettant en évidence les indicateurs métier essentiels : le volume total d'enregistrements CDR analysés, le nombre de suspects détectés par le modèle, le nombre de fraudes confirmées par validation humaine, ainsi que le taux global de fraude observé. Ces indicateurs sont mis à jour automatiquement à chaque rafraîchissement de la page grâce à des appels AJAX vers les endpoints du backend Flask.

La section intermédiaire propose des \textbf{visualisations graphiques interactives} générées avec \textbf{Chart.js}, parmi lesquelles : un graphique d'évolution temporelle de l'activité, un diagramme en anneau (\textit{doughnut chart}) illustrant la répartition des appels entrants et sortants, un histogramme représentant la distribution des appels par tranche horaire, ainsi qu'un graphique à barres horizontales détaillant les principaux indicateurs comportementaux retenus par le modèle.

L'identité visuelle de la page repose sur une \textbf{palette bleu-émeraude} cohérente avec la charte graphique globale, agrémentée d'effets de profondeur subtils (ombres légères, dégradés discrets) qui renforcent la perception de modernité et de fiabilité. Un \textbf{logo personnalisé} représentant une carte SIM associée à un indicateur d'alerte symbolise visuellement la thématique de la fraude SIM Box et confère à l'interface une identité distinctive.
...
\caption{Pages du tableau de bord} \label{tab:pages} \\
\hline
\textbf{Page} & \textbf{Description} \\
\hline
\endfirsthead

\multicolumn{2}{c}{\textit{Suite du tableau \ref{tab:pages}}} \\
\hline
\textbf{Page} & \textbf{Description} \\
\hline
\endhead
...
\includegraphics[width=0.9\textwidth]{images/liste_noire.png}
\end{figure}
...
\includegraphics[width=0.9\textwidth]{images/verification.png}
\end{figure}
