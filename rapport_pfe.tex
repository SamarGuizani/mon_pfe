...
\usepackage{array}
\usepackage{booktabs}
%\usepackage{mathptmx}        % Times New Roman pour tout le texte
\usepackage{setspace}        % déjà présent, garde-le
\onehalfspacing              % interligne 1.5 pour tout le rapport
...
\usepackage{float}
\definecolor{mauveFSG}{RGB}{102, 0, 153}
\usepackage{caption}
\captionsetup[table]{position=above}   % titre tableau → au-dessus
\captionsetup[figure]{position=below}  % titre figure → en dessous
\begin{document}

% ============================================================
% PAGE DE GARDE COMPLÈTE AVEC FILETS MAUVES
% ============================================================
\begin{titlepage}
\thispagestyle{empty}
...
\begin{center}
    ...
    % ==========================================
    % EN-TÊTE SUPÉRIEUR (Trois colonnes)
    % ==========================================
    \begin{minipage}[t]{0.45\textwidth}
        \begin{flushleft}
            ...
            la Recherche Scientifique\\[0.4em]
    ...
    \begin{minipage}[t]{0.15\textwidth}
        \centering
        \includegraphics[width=\linewidth, height=1.8cm, keepaspectratio]{images/image_2026-05-31_151921423.png}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
        \begin{flushright}
            \fontsize{10pt}{12pt}\selectfont
            A. U. : 2025-2026\\[0.3em]
            N° Ordre : LGLSI<\color{blue}N°\dots\color{black}>
        \end{flushright}
    \end{minipage}
    ...
    % ==========================================
    % Titre Principal
    {\fontsize{30pt}{36pt}\selectfont \textbf{Projet de Fin d'Études}} \\[0.6cm]

    % Présenté à
    {\fontsize{11pt}{14pt}\selectfont \textit{Présenté à :}} \\[0.4cm]
    {\fontsize{14pt}{18pt}\selectfont La Faculté des Sciences de Gabès} \\[0.8cm]

    % Département
    {\fontsize{14pt}{18pt}\selectfont \textbf{Département Informatique}} \\[0.8cm]

    % Diplôme visé
    {\fontsize{11pt}{14pt}\selectfont \textit{en vue de l'obtention du diplôme de}} \\[0.4cm]
    {\fontsize{13pt}{16pt}\selectfont \textbf{\color{blue} LICENCE GENIE LOGICIELS ET SYSTEMES D'INFORMATION}} \\[0.6cm]

    % ==========================================
    % PREMIÈRE DOUBLE LIGNE MAUVE (plus courte)
    % ==========================================
    \noindent\makebox[\textwidth]{%
    \color{mauveFSG}%
    \vbox{
        \hrule width 0.72\textwidth height 0.5pt 
        \vbox to 3pt{} 
        \hrule width 0.72\textwidth height 2.5pt 
        \vbox to 3pt{} 
        \hrule width 0.72\textwidth height 0.5pt
    }%
} \\[0.6cm]
    ...
    % Titre du Projet
    {\fontsize{14pt}{18pt}\selectfont \textbf{\color{blue} Approche hybride : Règles-Intelligence Artificielle pour la détection de la fraude Sim Box bypass à partir des CDR dans les réseaux de télécommunications 
}} \\[0.6cm]

    % ==========================================
    % DEUXIÈME DOUBLE LIGNE MAUVE (pleine largeur)
    % ==========================================
    \noindent\makebox[\textwidth]{%
    \color{mauveFSG}%
    \vbox{
        ...
        \vbox to 3pt{} 
        \hrule width \textwidth height 2.5pt 
        \vbox to 3pt{} 
        \hrule width \textwidth height 0.5pt
    }%
} \\[0.8cm]

    % Section Étudiants (Réalisé Par)
    {\fontsize{12pt}{15pt}\selectfont \textit{Réalisé Par :}} \\[0.4cm]
    {\fontsize{14pt}{18pt}\selectfont \textbf{\color{blue} Samar Guizani}} \\
    

    % Date et Jury
    {\fontsize{13pt}{16pt}\selectfont \textit{Soutenu le 12/06/2026}} \\
    {\fontsize{12pt}{15pt}\selectfont \textit{Devant le jury composé de :}} \\[0.8cm]
    ...
    \begin{tabular}{@{} p{0.6\textwidth} r @{}}
        \textbf{\color{blue}M. \quad Haifa Touati} & \textit{Présidente} \\
         \textbf{\color{blue}M. \quad Marwa Naas} & \textit{Examinatrice} \\
        \textbf{\color{blue}M. \quad Ridha Ejbali} & \textit{Encadrant} \\
...
\begin{center}
\textit{\textbf{À ma chère mère , Derbali Afef}}

{\Huge\textbf{À}} ma chère mère, source d'amour, de tendresse et de sacrifices, qui n'a jamais cessé de m'encourager dans toutes mes projets et qui a été pour moi un modèle de courage et de dévouement.

\end{center}
\begin{center}
\textit{\textbf{À mon cher père , Guizani Zahreddine}}

{\Huge\textbf{À}} mon cher père, qui m'a appris la valeur du travail bien fait, la rigueur et la persévérance. Ses sages conseils et son soutien  ont été les piliers de ma réussite tout au long de ces années d'études.


\end{center}

\begin{center}
\textit{\textbf{À mon frère Mohamed et mes  soeurs Arij , Rana }}


{\Huge\textbf{À}} mon frère et mes  sœurs , dont la présence chaleureuse et le soutien constant m'ont accompagnée dans les moments de doute comme dans les moments de joie.
...
{\Huge\textbf{À}} mes chers enseignants, qui m'ont transmis le savoir avec patience et passion tout au long de mon parcours academique.
...
{\Huge\textbf{À}} mes chers amis, avec qui j'ai partagé tant de moments précieux durant mon parcours universitaire. 
...
Je tiens tout d'abord  à exprimer ma profonde reconnaissance à la présidente du jury, \textbf{Mme Haifa Touati}, pour l'honneur qu'elle me fait de présider cette soutenance, ainsi qu'à la rapporteuse, \textbf{Mme Marwa Naas}, pour le temps précieux qu'elle a consacré à l'examen de ce travail.

Je tiens aussi  à remercier sincèrement mon encadrant académique à la \textbf{Faculté des Sciences de Gabes }, \textbf{M.Ridha Ejbali }, pour la qualité de son accompagnement, ses précieuses orientations et la pertinence de ses remarques. Sa rigueur scientifique et sa bienveillance ont été pour moi une source constante de motivation.
...
Mes remerciements vont aussi à l'ensemble de l'\textbf{équipe d'Elite Business}, qui m'a intégrée avec chaleur et professionnalisme, ainsi qu'à \textbf{tous les enseignants} de FSG pour la qualité de la formation dispensée durant mon parcours universitaire.

Mes plus sincères remerciements s'adressent enfin aux \textbf{honorables membres du jury} qui m'ont fait l'honneur d'accepter  ce modeste travail.

Enfin, à ma \textbf{famille} et à mes \textbf{amis}, je tiens à exprimer ma profonde gratitude pour leur soutien moral tout au long de cette aventure.
...
\noindent\rule[-0.2cm]{0.45cm}{1.2cm}\hspace{0.15cm}%
\raisebox{0.85cm}{\rule{0.85\textwidth}{0.5pt}}\\[0.3cm]
\noindent\hspace*{0.6cm}{\Huge\textbf{ABREVIATIONS}}\\
\begin{description}
...
\noindent\rule[-0.2cm]{0.45cm}{1.2cm}\hspace{0.15cm}%
\raisebox{0.85cm}{\rule{0.85\textwidth}{0.5pt}}\\[0.3cm]
\noindent\hspace*{0.6cm}{\Huge\textbf{INTRODUCTION GÉNÉRALE}}\\
\vspace{1cm}
...
Le \textbf{deuxième chapitre} est dédié à l'analyse et à la conception du système proposé. Il présente l'analyse des besoins, les diagrammes de modélisation UML, le modèle de données retenu et l'architecture technique générale. Nous y exposons également les choix technologiques 
effectués et les justifications qui les sous-tendent.
...
% On incrémente le compteur manuellement SANS afficher le titre par défaut
\refstepcounter{chapter} 
% On remet le compteur de section à 0 pour ce nouveau chapitre
...
\subsection{Approche basée sur l'apprentissage  supervisé}
...
\clearpage
\thispagestyle{fancy} % Pour avoir la ligne FSG en bas
\refstepcounter{chapter} % Le compteur passe à 2
\setcounter{section}{0}
...
acteurs et les fonctionnalités principales du système. Afin de
faciliter la lecture, le diagramme global est décliné ci-dessous en
cinq vues, une par acteur.

%--- Acteur 1 : Administrateur système ---------------------------
\subsubsection{Administrateur système}
L'administrateur système est responsable du bon fonctionnement de la
plateforme. Il gère les comptes utilisateurs, configure le système,
surveille le pipeline de traitement et consulte les logos. Il
intervient également sur le cycle de vie du modèle d'apprentissage :
entraînement, évaluation, déploiement et mise à jour. Les cas
d'utilisation sensibles incluent toujours l'authentification préalable.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/administrateur.png}
    \caption{Cas d'utilisation -- Administrateur système}
    \label{fig:uc_admin}
\end{figure}

%--- Acteur 2 : Analyste fraude ----------------------------------
\subsubsection{Analyste fraude}
L'analyste fraude exploite quotidiennement la plateforme pour
détecter, valider et documenter les cas de fraude SIM~Box. Il
consulte les alertes générées par le modèle, analyse les
caractéristiques d'une SIM suspecte, valide ou rejette les
prédictions, ajoute son retour (\emph{feedback}) pour enrichir
l'apprentissage et génère les rapports destinés aux équipes
opérationnelles.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/analystefraud.png}
    \caption{Cas d'utilisation -- Analyste fraude}
    \label{fig:uc_analyste}
\end{figure}

%--- Acteur 3 : API externe --------------------------------------
\subsubsection{API externe}
L'API externe représente les systèmes tiers qui interrogent la
plateforme pour obtenir un score de fraude en temps réel sur un
numéro donné. Ce cas d'utilisation déclenche en cascade la
construction des caractéristiques (\emph{features}) puis la
collecte des données CDR nécessaires au calcul.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{images/apiexterne.png}
    \caption{Cas d'utilisation -- API externe}
    \label{fig:uc_api}
\end{figure}

%--- Acteur 4 : Base de données PostgreSQL -----------------------
\subsubsection{Base de données PostgreSQL}
La base de données PostgreSQL est un acteur passif chargé de
persister les résultats du pipeline : caractéristiques calculées,
prédictions, listes noires et historique des décisions. Sa
disponibilité conditionne le bon fonctionnement de l'ensemble du
système.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/bd.png}
    \caption{Cas d'utilisation -- Base de données PostgreSQL}
    \label{fig:uc_db}
\end{figure}

%--- Acteur 5 : Système CDR --------------------------------------
\subsubsection{Système CDR}
Le système CDR est la source primaire
des données. Il alimente la plateforme avec les journaux d'appels
bruts produits par le réseau de l'opérateur, qui constituent la
matière première de la détection.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{images/système cdr.png}
    \caption{Cas d'utilisation -- Système CDR}
    \label{fig:uc_cdr}
\end{figure}
...
\begin{itemize}
    \item \underline{\textbf{Gestion des données :}} \\
    La classe \texttt{ConnecteurBaseDonnees} assure l'interface avec le stockage persistant, permettant l'extraction des \texttt{Donnees\_CDR}. Ces dernières regroupent les attributs techniques essentiels (MSISDN, IMEI, durée, horodatage) nécessaires à l'analyse.
    \vspace{0.3cm}
    ...
    \vspace{0.3cm}
\newpage
    \item \underline{\textbf{Intelligence Artificielle et Polymorphisme:}}\\
    Le système utilise une classe abstraite \texttt{ModeleFraude} qui définit les méthodes génériques d'entraînement et de prédiction. Cette structure permet d'implémenter différentes approches algorithmiques, telles que \texttt{ModeleXGBoost} et \texttt{ModeleLightGBM}, garantissant ainsi la flexibilité et la possibilité de comparer les performances des modèles.
    \vspace{0.3cm}

    \item \underline{\textbf{Alerte et Amélioration Continue :}}\\
    Une fois le score de risque produit,le \texttt{SystemeAlerte} évalue si le seuil critique est dépassé pour notifier l'analyste. Enfin, la \texttt{BoucleFeedback} permet d'enregistrer les validations humaines, créant un cycle d'apprentissage continu pour réentraîner et affiner la précision des modèles futurs.
\end{itemize}
...
\hline
liste\_noire\_fresh & 210 lignes & Suspects sur le test \\
\hline
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/ChatGPT Image 13 mai 2026, 10_30_29.png}
...
L'encadrant entreprise joue le rôle de \textbf{Product Owner} (il définit le besoin métier), l'encadrant académique celui de \textbf{Scrum Master} (il garantit la méthodologie), et l'étudiante constitue la \textbf{Scrum Team} chargée de la réalisation.
...
\hline
Product Owner & M. Ithar Lounissi (Elite Business) \\
\hline
Scrum Master & M. Ridha Ejbali (FSG) \\
\hline
...
\clearpage
\thispagestyle{fancy} % Pour avoir la ligne FSG en bas
\refstepcounter{chapter} % Le compteur passe à 3
\setcounter{section}{0}
...
Dans ce projet, le modèle est évalué de \textbf{deux manières distinctes} :

\begin{itemize}
\item \textbf{Test interne} (cette section) : on met de côté une partie du jeu d'apprentissage pour vérifier que le modèle apprend bien. Les données viennent du même mois que l'entraînement.
\item \textbf{Test indépendant} (section~\ref{tab:fresh-resultats}) : on teste le modèle sur 6~Go de données totalement neuves (mai 2026) que le modèle n'a \emph{jamais vues}. C'est le vrai examen de validation.
\end{itemize}

Pour le test interne, le jeu étiqueté a été partagé en 70~/~30, en gardant la même proportion de fraudes dans chaque sous-ensemble.

\begin{table}[H]
\centering
\caption{Répartition du jeu étiqueté pour le test interne}
\label{tab:split-train-test}
\vspace{0.3cm}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Sous-ensemble} & \textbf{Proportion} & \textbf{Nombre de numéros} & \textbf{Dont fraudes} \\
\hline
Entraînement & 70~\% & 2 997 975 & 106 \\
\hline
Test interne & 30~\% & 1 284 847 & 46 \\
\hline
\textbf{Total} & \textbf{100~\%} & \textbf{4 282 822} & \textbf{152} \\
\hline
\end{tabular}
\end{table}

\subsection{Entraînement du modèle}
Le modèle XGBoost est entraîné sur les 70~\% avec les hyperparamètres suivants : 300 arbres, profondeur maximale de~6, taux d'apprentissage de 0,1. Le paramètre \texttt{scale\_pos\_weight} est ajusté automatiquement pour gérer le déséquilibre des classes.

\subsection{Résultats du test interne}
Le modèle est évalué sur les 30~\% mis de côté (1~284~847 numéros, dont 46 vraies fraudes). Le tableau~\ref{tab:resultats-ml} présente les métriques obtenues.

\begin{table}[H]
\centering
\caption{Résultats du modèle XGBoost sur le test interne}
...
\noindent
\textbf{Lecture :} le modèle attrape 44 fraudes sur 46 (Recall 96~\%) et génère 14 faux positifs (Précision 76~\%). L'AUC de 1,00 confirme qu'il sépare parfaitement les deux classes. Ces résultats valident la phase d'apprentissage~; le véritable examen sur données neuves est présenté plus loin (section~\ref{tab:fresh-resultats}).

\section{DÉVELOPPEMENT DU TABLEAU DE BORD}

\subsection{Architecture du frontend}
La page d'accueil, illustrée par la figure~\ref{fig:dashboard}, constitue le point d'entrée principal de la plateforme à l'ouverture du site. Elle a été conçue pour offrir à l'analyste une \textbf{vue d'ensemble synthétique et instantanée} de l'état du système de détection .
\newpage
Sa partie supérieure présente une série de \textbf{cartes statistiques } mettant en évidence les indicateurs métier essentiels : le volume total d'enregistrements CDR analysés, le nombre de suspects détectés par le modèle, le nombre de fraudes confirmées par validation humaine, ainsi que le taux global de fraude observé. Ces indicateurs sont mis à jour automatiquement à chaque rafraîchissement de la page grâce à des appels AJAX vers les endpoints du backend Flask.
Cette page joue ainsi un double rôle : elle informe l'utilisateur sur la santé globale du système de détection et l'oriente intuitivement vers les modules d'investigation accessibles depuis la barre latérale de navigation.
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/dash.png}
...
\hline
\textbf{Section} & \textbf{Page} & \textbf{Description} \\
\hline
\multirow{1}{*}{Main} & Dashboard & Statistiques globales et indicateurs clés du système \\
\hline
\multirow{5}{*}{Détection}
 & Résultats ML & Métriques d'évaluation du modèle XGBoost \\
\cline{2-3}
 & Test Données Fresh & Résultat du test indépendant sur 6 Go neufs (mai 2026) \\
\cline{2-3}
 & Prédiction IA & Prédiction en direct sur un MSISDN saisi par l'utilisateur \\
\cline{2-3}
 & Liste Noire & Suspects détectés avec filtres, recherche et pagination \\
\cline{2-3}
 & IMEI des Suspects & Analyse des IMEI partagés (signature SIM Box) \\
\hline
\multirow{1}{*}{Analyse}
 & Fraud Rules & Documentation interactive des règles métier \\
\hline
\multirow{2}{*}{Administration}
 & Vérification Manuelle & Validation ou rejet des suspects (réservée admin) \\
\cline{2-3}
 & Gestion Utilisateurs & Création et administration des comptes (admin) \\
\hline
\end{tabular}
\end{table}
...
\subsection{Page IMEI des suspects}
La page \emph{IMEI des Suspects} regroupe l'analyse des identifiants matériels (IMEI) associés aux numéros suspects. Elle met en évidence les IMEI utilisés par plusieurs MSISDN, ce qui constitue une signature forte d'une SIM~Box (plusieurs cartes SIM tournant sur un même appareil). La figure~\ref{fig:imei} illustre cette page.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/imei.png}
\caption{Page IMEI des suspects}
\label{fig:imei}
\end{figure}

\subsection{Page de test sur données fraîches}
La page \emph{Test Données Fresh} présente les résultats de la validation finale du système sur un lot de 6~Go de CDR neufs (mai 2026), jamais vus par le modèle pendant l'apprentissage. Elle affiche notamment la précision réelle obtenue (100~\%), la décomposition géographique des 210 fraudeurs détectés, ainsi que la liste complète des numéros suspectés avec leur probabilité prédite. La figure~\ref{fig:testfresh} illustre cette page.
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/freshtrain.png}
\caption{Page de test sur données fraîches}
\label{fig:testfresh}
\end{figure}
\subsection{Système d'authentification}
Le module d'authentification propose un parcours complet : inscription avec saisie de l'email, envoi d'un code de vérification à six chiffres par email via Gmail SMTP, validation du code, connexion par email et mot de passe, ainsi qu'une procédure de récupération du mot de passe en cas d'oubli. La figure~\ref{fig:authentification} présente l'interface de connexion.

% APRÈS (correct, sans espaces vides)
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/authentificationpage.png}
...
\subsection{Test indépendant sur données fraîches}

Afin de valider la robustesse du système au-delà du jeu d'entraînement,
un test indépendant a été réalisé sur un nouveau lot de données fourni
par l'entreprise : \textbf{6 gigaoctets de CDR compressés
(environ 30 Go décompressés) couvrant la période du 8 au 12 mai 2026}.
Ce mois étant postérieur à la période d'apprentissage, le modèle n'avait
\textbf{jamais vu ces données}, ce qui en fait un véritable examen
opérationnel.

\subsubsection{Préparation du jeu de test}
Les 6 Go ont été importés dans une table dédiée
(\texttt{cdr\_data\_fresh}), totalement séparée du jeu d'entraînement
afin de garantir l'absence de contamination. Le pipeline de
\emph{Feature Engineering} a ensuite été ré-exécuté pour générer les
16 indicateurs comportementaux dans la table
\texttt{features\_msisdn\_fresh}.

\begin{table}[H]
\centering
\caption{Volumétrie du jeu de test indépendant}
\label{tab:fresh-volume}
\vspace{0.3cm}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|l|}
\hline
...
\hline
Période couverte & 8 -- 12 mai 2026 \\
\hline
Volume CDR (compressé / brut) & 6 Go / 30 Go \\
\hline
Nombre de lignes CDR importées & 213 695 147 \\
\hline
Nombre de numéros (MSISDN) analysés & 3 971 333 \\
\hline
Durée d'import dans PostgreSQL & 2 h  \\
\hline
Durée du Feature Engineering & 2 h 54 min \\
\hline
\end{tabular}
...
Le modèle XGBoost déjà entraîné a été appliqué aux 3 971 333 numéros
\emph{sans réentraînement}. Les prédictions ont ensuite été confrontées
à la liste de fraudeurs confirmés manuellement par l'entreprise sur la
même période, afin de mesurer la précision réelle du système sur des
données opérationnelles fraîches.

\subsubsection{Résultats obtenus}

Sur ce jeu indépendant, le modèle a détecté \textbf{210 numéros
suspects}. La confrontation avec les listes de l'entreprise a confirmé
que \textbf{les 210 numéros détectés sont tous de véritables
fraudeurs}, soit une \textbf{précision de 100~\% sans aucun faux
positif}.

\begin{table}[H]
\centering
\caption{Résultats du test indépendant sur les données fraîches}
\label{tab:fresh-resultats}
\vspace{0.3cm}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|l|}
\hline
...
\hline
Numéros analysés & 3 971 333 \\
\hline
Suspects détectés par le modèle & 210 \\
\hline
Confirmés vrais fraudeurs par l'entreprise & 210 / 210 \\
\hline
Faux positifs & 0 \\
\hline
Précision réelle & 100~\% \\
\hline
\end{tabular}
\end{table}

\subsubsection{Découverte d'une nouvelle zone de fraude}
La décomposition géographique des 210 fraudeurs détectés a révélé un
résultat opérationnel majeur. La liste initiale de l'entreprise couvrait
une seule zone surveillée, contenant 207 fraudeurs confirmés. Le modèle
a retrouvé 106 de ces fraudeurs dans cette zone connue, mais a
également signalé \textbf{104 fraudeurs supplémentaires situés dans une
zone géographique non couverte} par les vérifications manuelles de
l'entreprise. Tous ces 104 numéros ont été confirmés \emph{a posteriori}
comme de vrais fraudeurs.

\begin{table}[H]
\centering
\caption{Décomposition géographique des 210 suspects détectés}
\label{tab:fresh-zones}
\vspace{0.3cm}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|c|}
\hline
\textbf{Zone géographique} & \textbf{Nombre de fraudeurs détectés} \\
\hline
Zone surveillée par l'entreprise & 106 \\
\hline
Nouvelle zone (non couverte initialement) & 104 \\
\hline
...
\subsubsection{Interprétation et impact opérationnel}
Ce test indépendant valide la solution sur trois plans complémentaires.
Premièrement, sur le plan \textbf{statistique}, l'absence totale de faux
positifs sur 3,9 millions de numéros analysés démontre la pertinence
des indicateurs choisis et la robustesse du modèle entraîné.
Deuxièmement, sur le plan de la \textbf{généralisation}, la performance
parfaite sur un mois jamais vu pendant l'apprentissage écarte le risque
de sur-apprentissage. Troisièmement, et c'est l'apport métier le plus
significatif, la mise en évidence de 104 fraudeurs dans une zone non
surveillée illustre la valeur ajoutée du \emph{Machine Learning} par
rapport aux vérifications manuelles : le modèle ne se contente pas de
reproduire les détections existantes, il \textbf{étend automatiquement
le périmètre de détection} à des cas que les analystes n'avaient pas
encore explorés.
...
\hline
Précision sur données fraîches & Non mesurée & 100~\% (210 / 210) \\
\hline
Couverture géographique & Une seule zone & Étendue à une nouvelle zone \\
\hline
\end{tabular}
\end{table}
...
\newpage
\section{CONCLUSION}
Ce troisième chapitre a présenté la réalisation concrète de notre projet ainsi que les résultats obtenus. Le pipeline de Feature Engineering a permis de transformer un volume considérable de données brutes en une représentation compacte exploitable par le Machine Learning. Le modèle XGBoost atteint un F1-Score de 0,85 avec un Recall de 96\% sur le jeu d'entraînement. Surtout, le test indépendant mené sur 6 Go de données fraîches (mai 2026) a confirmé la robustesse de la solution avec une \textbf{précision de 100~\% (210 vrais fraudeurs sur 210 détectés)} et a permis de \textbf{découvrir une nouvelle zone de fraude} jusqu'alors non surveillée par l'entreprise. Le tableau de bord développé offre une expérience utilisateur moderne et intègre l'innovation principale du Human-in-the-Loop, permettant un cycle d'amélioration continue de la détection.
...
Le code source complet est disponible publiquement à l'adresse :\\
\url{https://github.com/SamarGuizani/mon_pfe}

\section*{ANNEXE B : Captures d'ecran}
Les captures d'écran détaillées du tableau de bord sont fournies dans le dossier \texttt{images/} associe à ce rapport.
...
\begin{center}
    ...
    % TITRE EN HAUT
    % ==========================================
    {\fontsize{13pt}{16pt}\selectfont \textbf{\color{blue} Approche hybride : Règles-Intelligence Artificielle pour la détection de la fraude Sim Box bypass à partir des CDR dans les réseaux de télécommunications }} \\[0.4cm]

    % Première double ligne mauve (courte)
    \noindent\makebox[\textwidth]{%
        \color{mauveFSG}%
        \vbox{
            \hrule width 0.55\textwidth height 0.5pt
            \vbox to 3pt{}
            \hrule width 0.55\textwidth height 2.5pt
            \vbox to 3pt{}
            \hrule width 0.55\textwidth height 0.5pt
        }%
    } \\[0.3cm]
    ...
    % Noms des auteurs
    {\fontsize{12pt}{15pt}\selectfont \textbf{\color{blue} Samar Guizani}} \\[0.3cm]

    % Deuxième double ligne mauve (courte)
    \noindent\makebox[\textwidth]{%
        \color{mauveFSG}%
        \vbox{
            \hrule width 0.45\textwidth height 0.5pt
            \vbox to 3pt{}
            \hrule width 0.45\textwidth height 2.5pt
            \vbox to 3pt{}
            \hrule width 0.45\textwidth height 0.5pt
        }%
    } \\[0.8cm]
...
% ============================================================
% SECTION ARABE COMPATIBLE PDFLATEX (خلاصة + الكلمات المفتاحية)
% ============================================================
\begin{otherlanguage}{arabic}
\vspace{-0.2cm}

% Titre du Résumé aligné à droite nativement
\noindent{\fontsize{13pt}{15pt}\selectfont
\underline{\textit{\textbf{\color{blue}الملخص:}}}}
% Corps du texte
{\fontsize{10.5pt}{14pt}\selectfont
يهدف مشروع تخرجنا هذا إلى تصميم وتطوير نظام ذكي للكشف عن الاحتيال عبر أجهزة الـ Box SIM في قطاع الاتصالات. وتعتمد المقاربة المعتمدة على الجمع بين قواعد العمل الخاصة بالمؤسسة وخوارزميات تعلم الآلة Learning Machine  مثل XGBoost وIsolation Forest حيث تم دمجها بالكامل في لوحة تحكم ويب احترافية تدعم نظام المصادقة، وإدارة الصلاحيات، والتحقق البشري. تم التحقق من كفاءة التطبيق واختباره على قاعدة بيانات ضخمة تحتوي على 741 مليون سجل من تفاصيل المكالمات (CDR) بحجم إجمالي يبلغ 83 جيجابايت، وذلك بعد مرحلة هندسة الميزات التي أسفرت عن استخراج 16 مؤشرًا سلوكيًا لكل رقم هاتف. وقد حقق نموذج (XGBoost) مقياس دقة (F1-Score) قدره 0.85 مع نسبة استدعاء (Recall) تصل إلى 96\% على مجموعة بيانات الاختبار. ويكمن الابتكار الرئيسي لهذا المشروع في إدراج وحدة تفاعل "العنصر البشري في المسار" (Human-in-the-Loop)، مما يتيح التطوير والتحسين المستمر للنموذج.
}

\vspace{0.4cm}

% Mots-clés alignés à droite nativement
\noindent{\fontsize{10.5pt}{13pt}\selectfont
\underline{\textit{\textbf{\color{blue}الكلمات المفتاحية:}}}} الاحتيال عبر أجهزة الـ Box SIM  الاتصالات، تعلم الآلة , Learning Machine  XGBoost، البيانات الضخمة Data big ,  Flask ,  PostgreSQL.
...
% RÉSUMÉ FRANÇAIS
% ==========================================
\noindent \underline{\textit{\textbf{\color{blue}Résumé :}}} Ce projet de fin d'études vise la conception et la réalisation d'un système intelligent de détection de fraude SIM Box dans les télécommunications. L'approche retenue combine les règles métier de l'entreprise avec des algorithmes de Machine Learning (XGBoost et Isolation Forest), le tout déployé dans un tableau de bord web professionnel avec authentification, gestion des rôles et validation humaine. L'application a été validée sur un jeu de données de 741 millions d'enregistrements CDR (83 Go), après une phase de Feature Engineering qui a produit 16 indicateurs comportementaux par numéro. Le modèle XGBoost atteint un F1-Score de 0,85 avec un Recall de 96\% sur le jeu de test. L'innovation principale du projet réside dans l'intégration d'un module Human-in-the-Loop permettant l'amélioration continue du modèle.
...
% MOTS CLÉS FRANÇAIS
% ==========================================
\noindent \underline{\textit{\textbf{\color{blue}Mots clés :}}} fraude SIM Box, télécommunications, Machine Learning, XGBoost, Big Data, Flask, PostgreSQL.
...
% KEY-WORDS ANGLAIS
% ==========================================
\noindent \underline{\textit{\textbf{\color{blue}Key-words:}}} SIM Box fraud, telecommunications, Machine Learning, XGBoost, Big Data, Flask, PostgreSQL.
